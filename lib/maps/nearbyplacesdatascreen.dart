...
class Nearbyplacesdata extends StatefulWidget {
  final String fsqid;
  const Nearbyplacesdata({super.key,
  required this.fsqid,
  });
     ...
     try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          name = data["name"];
          placeicon="${data["categories"][0]["icon"]["prefix"]}bg_64${data["categories"][0]["icon"]["suffix"]}";
        });
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          for (int i = 0; i < photoData.length; i++) {
            photoslinks.add(photoData[i]["prefix"] + "original" + photoData[i]["suffix"]);
          }
        });
        ...
        setState(() {
          photoslinks = ['Failed to load photos. Status code: ${response.statusCode}'];
        });    
      }
    } 
    catch (e) {
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              Row(
                children: [
                  Expanded(child: Text(name, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 45),)),
                  SizedBox(width: 10),
                  Image.network(
                    placeicon,
                    height: 55,
              ...
              photoslinks.isNotEmpty
                            ...
                            Align(
                              ...
                              child: const Text("Reviews", style: TextStyle(fontSize: 25, fontWeight: FontWeight.w600, color: Colors.red),)),
